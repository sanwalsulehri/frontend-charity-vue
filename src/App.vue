<template>
   <Disclosure as="nav" class="bg-gray-800 w-full z-[99999] relative" v-slot="{ open }">
    <div class="mx-auto max-w-7xl w-full px-2 sm:px-6 lg:px-8">
      <div class="relative w-full flex h-16 items-center justify-between">
        <div class="absolute sm:w-0 inset-y-0 left-0 flex items-center sm:hidden">
          <!-- Mobile menu button-->
        ...
        <div class="flex flex-1 items-center sm:items-stretch sm:justify-start">
          ...
          <div class="hidden sm:ml-6 sm:block">
            <div class="flex space-x-4">
              <router-link v-for="item in navigation" :key="item.name" :href="item.href" :class="[item.current ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'rounded-md px-3 py-2 text-sm font-medium']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</router-link>
            </div>
          </div>
        </div>
