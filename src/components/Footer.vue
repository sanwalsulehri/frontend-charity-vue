<template lang="">
    <footer aria-labelledby="footer-heading" class="">
      ...
      <div class="mx-auto  max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="py-20 borderTop xl:grid xl:grid-cols-3 xl:gap-8">
          ...
          <div class="mt-12 md:mt-16 xl:mt-0">
            ...
            <form class="mt-2 flex sm:max-w-md">
              <input id="email-address" type="text" autocomplete="email" required="" placeholder="Enter email" ria-label="Email address" class="block w-full rounded-md bg-white px-4 py-2 text-base  outline-1   placeholder:text-gray-600 focus:ring-2 outline-none focus:ring-[#059669]" />
              <div class="ml-4 shrink-0">
                <button type="submit" class="flex w-full items-center justify-center rounded-md border border-transparent bg-emerald-600 px-4 py-2 text-base font-medium text-white shadow-xs hover:bg-emerald-700 focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:ring-offset-gray-900 focus:outline-hidden">Sign up</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <div class="flex justify-center gap-x-6 md:order-2">
        <a v-for="item in navigation" :key="item.name" :href="item.href" class="text-emerald-400 hover:text-emerald-300">
          <span class="sr-only">{{ item.name }}</span>
          <component :is="item.icon" class="size-6" aria-hidden="true" />
        </a>
      </div>
      <p class=" text-center text-sm/6 text-gray-400 md:order-1 md:mt-0">&copy; 2024 Your Company, Inc. All rights reserved.</p>
    </div>
      </div>
    </footer>
</template>
